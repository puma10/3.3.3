...
if __name__ == "__main__":
    ...
    log.info("starting with {} shelters".format(num_shelter) )


    log.info("Creating Shelter: NYC Pat Orphanage and New Orleans Pet Hotel(Transient)")
    nyc = Shelter(name="NYC Pet Orphanage", website="http://www.nyc-pet-orphanage.com")
    neworl = Shelter(name="New Orleans Pet Hotel", website="http://www.neworleanspethotel.com")


    # we can verify that cat and dog do not have ids yet.
    assert nyc.id == None
    assert neworl.id == None
    log.info("Type of NYC.id is {}. Type of NewOrld.id is {}".format(type(nyc.id), type(neworl.id) ))


    # add them to the session and commit to save to db
    log.info("Adding shelters.")
    db_session.add_all( [nyc, neworl] )
    db_session.commit()
    ...
    assert nyc.id != None
    assert neworl.id != None
    log.info("Now the type of nyc.id is {} and type of neworl.id is {}".format(nyc.id, neworl.id ))
    ...
    num_pet = db_session.query(Pet).count()

    log.info("starting with {} pets".format(num_pet) )


    log.info("Creating Pets: Sue and Thomas(Transient)")

    thomas = Pet(name="Thomas", age=5, adopted=0, shelter_id="NYC Pet Orphanage", breed_id = golden.id)
